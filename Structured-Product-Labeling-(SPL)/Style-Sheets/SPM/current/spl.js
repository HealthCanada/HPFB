...
function moveToFirstTD(firstDivElement, secondDivElement, innerElement, lastClonedNode) {
    ...
    while (next) {
        ...
        if (copyCompleteElement) {
            ...
            /* Handling lists elements(ul,ol) separately - #1393 */
        ...
        if (lastClonedNode) {
            if (secondDivElement.offsetHeight > halfLength) {
                // TODO Decide whether to move the last element to left
                if (! isListElement)
...
function toggleMixin() {
    if (! _mixinStyleSheet) {
        ...
        _mixinStyleSheet.insertRule(".spl .Mixin { display:none; }", 0); else if (_mixinStyleSheet.addRule)
        _mixinStyleSheet.addRule(".spl .Mixin", "display:none;", -1); else
        return;
        _mixinState = -1;
    } else {
